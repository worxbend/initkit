...
object CommandsExecutorTests extends TestSuite:
  ...
  val tests: Tests = Tests:
    test("generates shell-mode command specs from config example"):
      ...
      assert(commands.map(_.sudo) ==
        Vector(SudoMode.Required, SudoMode.Required, SudoMode.Disabled))
    ...
    test("apply mode does not execute skipped command items"):
      ...
      assert(outcome ==
        PlanOperationOutcome.Completed(Vector("ran 2 command(s), skipped 1 command(s)")))
      assert(executor.calls == commands)
    ...
    test("dry-run previews shell command text without executing"):
      ...
      assert(dryRun.actions == Vector(
        DryRunAction.Message(
          "skip command 'enable-docker': required command 'systemctl' is not available"
        ),
  ...
  private def commandsOperation(name: String): PlanOperation = planOperation(name)
  ...
  ): Vector[CommandSpec] = CommandsExecutor
  ...
  private def skippedSteps(
      hostFacts: HostFacts,
      policy: ExecutionPolicy
  ): Vector[CommandExecutionStep.Skipped] = CommandsExecutor
    .executionSteps(commandsPlanOperation("post-install"), policy, hostFacts)
    .collect:
      case step: CommandExecutionStep.Skipped => step

  private def shellCommands(commands: Vector[CommandSpec]): Vector[String] = commands.map:
    command =>
      command.invocation match
        case CommandInvocation.Shell(argument, shell) =>
          assert(shell == Vector("/bin/sh", "-c"))
          argument.value
        case CommandInvocation.Direct(_) => fail("expected shell command")

  private def dryRunShell(command: String, sudo: Boolean): DryRunAction = DryRunAction.Command(
    argv = Vector(command),
    shell = Some("/bin/sh -c"),
    sudo = sudo,
