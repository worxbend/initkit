...
final class BinaryDownloadsExecutor(
  ...
  def install(
    ...
    case ExecutionRunMode.Apply => applyDownloads(operation)
  ...
  private def applyDownloads(
    ...
    failures.headOption match
      ...
      case None => PlanOperationOutcome.Completed(Vector(completionDetail(installed)))

  private def runItems(
      operation: InstallerPlanOperation[InstallerSpec.BinaryDownloads]
  ): Vector[Either[BinaryDownloadFailure, Unit]] = operation.execution.mode match
    case PlanEntryExecutionMode.Sequential =>
      runItemsSequentially(operation.spec.items, operation.execution.failFast)
    case PlanEntryExecutionMode.Parallel => runItemsInParallel(
        operation.spec.items,
        operation.execution.maxConcurrency,
        operation.execution.failFast
      )
  ...
  private def runItemsInParallel(
    ...
    if failFast then
      try
        ...
        case error: BinaryDownloadFailFast => Vector(Left(error.failure))
  ...
  private def installFile(
    ...
    for
      _ <- files.setMode(source, item.mode).left.map(error =>
        BinaryDownloadFailure.File(item.name, error.message)
      )
      _ <- files.moveIntoPlace(source, destination).left.map(error =>
        BinaryDownloadFailure.File(item.name, error.message)
      )
    yield ()
  ...
  private def download(
      item: BinaryDownloadItem,
      tempPath: Path
  ): Either[BinaryDownloadFailure, Unit] = httpClient.download(item.url, tempPath, config) match
    case Right(_)    => Right(())
    case Left(error) => Left(BinaryDownloadFailure.Download(item.name, error.message))

  private def verifyChecksum(
      item: BinaryDownloadItem,
      tempPath: Path
  ): Either[BinaryDownloadFailure, Unit] = item.checksum match
    case None           => Right(())
    case Some(checksum) => BinaryDownloadsExecutor.checksumHex(tempPath, checksum.algorithm) match
  ...
  private def completionDetail(installed: Int): String = installed match
...
private object BinaryTarGzExtractor:
  ...
  private def extractTarGzFile(
    ...
    extractMatchingEntry(
      archivePath,
      destination,
      entry => entry.strippedName(stripComponents) == selectedPath
    )
      .flatMap:
        case SelectedTarEntry.Found    => Right(())
        case SelectedTarEntry.NotFound => extractMatchingEntry(
            archivePath,
            destination,
            entry => entry.name == selectedPath
          ).flatMap:
            case SelectedTarEntry.Found    => Right(())
  ...
  private def readTar(
    ...
    while !done do
      readHeader(input, header) match
        case HeaderRead.EndOfArchive => done = true
        case HeaderRead.Header       =>
          ...
          if matches(entry) then
            if entry.isRegularFile then
              ...
              failure = Some(
                BinaryTarGzExtractionError(s"archive member '${entry.name}' is not a regular file")
              )
              done = true
    ...
    failure match
      ...
      case None if selected => Right(SelectedTarEntry.Found)
      case None             => Right(SelectedTarEntry.NotFound)

  private def readHeader(input: InputStream, header: Array[Byte]): HeaderRead =
    readFullyOrEof(input, header) match
      case 0                                            => HeaderRead.EndOfArchive
      case TarBlockSize if header.forall(_ == 0.toByte) => HeaderRead.EndOfArchive
      case TarBlockSize                                 => HeaderRead.Header
      case read => throw EOFException(s"truncated tar header after $read byte(s)")
  ...
  private def normalizeTarPath(value: String): String = value
    .replace('\\', '/')
    .split("/")
    .filter(component => component.nonEmpty && component != ".")
    .mkString("/")
  ...
  private final case class TarEntry(name: String, size: Long, typeFlag: Byte):
    def isRegularFile: Boolean = RegularFileTypes.contains(typeFlag)
  ...
  private object TarEntry:
    ...
    def fromHeader(header: Array[Byte]): TarEntry = TarEntry(
...
enum BinaryDownloadHttpError:
  ...
  def message: String = this match
    case InvalidUrl(url, detail)                                 => s"invalid URL '$url': $detail"
    case HttpStatus(url, statusCode, body) if body.trim.nonEmpty =>
      s"HTTP $statusCode from $url: ${body.trim.take(200)}"
    case HttpStatus(url, statusCode, _) => s"HTTP $statusCode from $url"
    case Transport(url, detail)         => s"failed to download $url: $detail"
    case Interrupted(url)               => s"interrupted while downloading $url"

object BinaryDownloadHttpClient:

  val Sttp: BinaryDownloadHttpClient = new BinaryDownloadHttpClient:
    override def download(
      ...
      try
        Uri.parse(url) match
          case Left(error) => Left(BinaryDownloadHttpError.InvalidUrl(url, error))
          case Right(uri)  =>
        ...
        case NonFatal(error) => Left(BinaryDownloadHttpError.Transport(
            url,
...
object BinaryDownloadFiles:
  ...
  val Jvm: BinaryDownloadFiles = new BinaryDownloadFiles:
    override def createTempDownload(
        itemName: String,
        destination: Path
    ): Either[BinaryDownloadFileError, Path] = safely:
      val absoluteDestination = destination.toAbsolutePath.normalize()
      val parent              =
        Option(absoluteDestination.getParent).getOrElse(Path.of(".").toAbsolutePath.normalize())
      Files.createDirectories(parent)
    ...
    private def safely[A](body: => A): Either[BinaryDownloadFileError, A] =
      try Right(body)
        ...
        case error: IOException                   => Left(BinaryDownloadFileError(error.getMessage))
        case error: SecurityException             => Left(BinaryDownloadFileError(error.getMessage))
        case error: UnsupportedOperationException => Left(BinaryDownloadFileError(error.getMessage))
...
object BinaryDownloadsExecutor:
  ...
  ): Either[BinaryDownloadFileError, String] =
  ...
  def normalizeChecksum(value: String): String = value.trim.toLowerCase

  def permissionsFromMode(mode: String): Either[String, Set[PosixFilePermission]] =
    val trimmed            = mode.trim
    val withoutLeadingZero =
      if trimmed.length == 4 && trimmed.startsWith("0") then trimmed.drop(1) else trimmed
  ...
  private def dryRunActions(
      item: BinaryDownloadItem,
      files: BinaryDownloadFiles
  ): Vector[DryRunAction] =
    val destination = Path.of(item.destination)
    val tempPath    = files.previewTempDownloadPath(item.name, destination)
    val checksum    = item.checksum.map(value =>
      s" with ${checksumName(value.algorithm)} verification"
    ).getOrElse("")
    val archiveSuffix = item.archive.map(_ => " archive").getOrElse("")

    Vector(
      DryRunAction.Message(
        s"download binary$archiveSuffix '${item.name}' from ${item.url} to $tempPath$checksum"
      ),
      DryRunAction.FileWrite(
        item.destination,
        Some(item.mode),
  ...
  ): Unit = if (value & bit) != 0 then permissions += permission
