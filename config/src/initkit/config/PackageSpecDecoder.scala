...
object PackageSpecDecoder:
  ...
  def isPackageKind(kind: String): Boolean =
    PackageKinds.contains(kind)
  ...
  private def decodePackageFields(
    ...
    kind match
      ...
      case "dnf-packages" =>
        decodeInstallList(fields, specAt, entryName).map(PackageSpec.Dnf.apply)
      ...
      case "flatpak-packages" =>
        buildFlatpakSpec(fields, specAt, entryName)
      case "snap-packages" =>
        decodeSnapInstallList(fields, specAt, entryName).map(PackageSpec.Snap.apply)
      case other =>
        Left(Vector(error(specAt, s"unsupported package kind '$other'")))
  ...
  private def decodeInstallList(
    ...
    fields.get("install") match
      case Some(RawYaml.SequenceValue(items)) if items.nonEmpty =>
        sequence(items.zipWithIndex.map((item, index) => decodeString(item, s"$specAt.install[$index]")))
      case Some(RawYaml.SequenceValue(_)) =>
        Left(Vector(error(s"$specAt.install", withEntryName(entryName, "must contain at least one package"))))
      case Some(other) =>
        Left(Vector(error(s"$specAt.install", s"must be a non-empty sequence, found ${kindOf(other)}")))
      case None =>
        Left(Vector(error(s"$specAt.install", withEntryName(entryName, "is required and must contain at least one package"))))

  private def decodeSnapInstallList(
      fields: VectorMap[String, RawYaml],
      specAt: String,
      entryName: Option[String]
  ): Either[Vector[ManifestValidationError], Vector[SnapPackage]] =
    fields.get("install") match
      case Some(RawYaml.SequenceValue(items)) if items.nonEmpty =>
        sequence(items.zipWithIndex.map((item, index) => decodeSnapPackage(item, s"$specAt.install[$index]")))
      case Some(RawYaml.SequenceValue(_)) =>
        Left(Vector(error(s"$specAt.install", withEntryName(entryName, "must contain at least one package"))))
      case Some(other) =>
        Left(Vector(error(s"$specAt.install", s"must be a non-empty sequence, found ${kindOf(other)}")))
  ...
  private def decodeSnapPackage(
      raw: RawYaml,
      at: String
  ): Either[Vector[ManifestValidationError], SnapPackage] =
    raw match
      ...
      case other => Left(Vector(error(at, s"must be a package name or mapping, found ${kindOf(other)}")))
